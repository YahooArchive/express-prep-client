...
module.exports = minifyFunction(ns);

function ns(ready, args) {
    var d = document,
        global = window,
    ...
    function done() {
        ...
        while ((m = ns._promisesQueue.shift())) {
    ...
    setTimeout(function () {
        // in case there is nothing to patch, we want to maintain the async nature
        // of the thenable here.
        if (ns._pending <= 0) {
            done();
        }
    }, 0);
