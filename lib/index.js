...
function ExpressPrepClient(app, config) {
    ...
    app.expose(require('./thenable-shim'), 'window.ExpressPrepClient', { cache: true });
    app.expose(function () { return ExpressPrepClient.apply(this, [function () {}]); },
            'app.init', { cache: true });
    // exposing the custom triggers
    Object.keys(config || []).forEach(function (ns) {
        if (typeof config[ns] === 'function') {
            app.expose(function namedCallee() { return ExpressPrepClient.apply(this, [namedCallee.ready, arguments]); },
                    ns, { cache: true });
